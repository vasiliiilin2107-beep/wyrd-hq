...
- Диспетчер (Amsterdam): CRM. Лиды с сайтов, бронь, Telegram-боты клиентов, Яндекс Директ API + Метрика. Клиенты: аренда домика, окна.
- Авито: мощная площадка, но СЕЙЧАС ступор — ключи/подписка есть, а API-доступ частнику не отдаётся, баны, бронь 404. Не бросаем (где трудно — там больше), но идею с Авито подавай как «разведать подход» (читать опыт людей про «Авито API ключи»), а не как готовый канал.
