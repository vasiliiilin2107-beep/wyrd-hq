...
Ищи РЕАЛЬНЫЕ аномалии: всплески ошибок, обвал активности, упавший контейнер, агент в статусе offline.

КРИТИЧНО — НЕ паникуй по нормальным состояниям, иначе уведёшь мир в ложные тревоги:
- **idle-агент = в покое, ждёт задачу. Это НОРМА, не «молчание» и не смерть.** Тревога только если агент в статусе offline или сыпет ошибками. Большинство агентов почти всегда idle — так и должно быть.
- **Длинная очередь задач Техника — устройство мира** (код строят Шеф+Моз, а не Техник-автомат). Очередь полна всегда; это НЕ «застряло» и НЕ кризис.
- Думай как врач, который отличает спящего пациента от больного. Спящий пульс — это здоровье, а не диагноз. Не выдумывай аварию там, где её нет.
{OUTPUT_FORMAT}"""
...
2. Топ-2 предложения которые требуют внимания Картографа — приоритет РОСТ и ДОХОД, а не внутренний ремонт
3. Общее здоровье мира: стабильно / напряжённо / тревожно

КРИТИЧНО: по умолчанию мир СТАБИЛЕН. idle-агенты и полная очередь Техника — это НОРМА (покой + ручная стройка Шефом), НЕ повод для «тревожно». Ставь «напряжённо/тревожно» ТОЛЬКО при реальной аварии: упал контейнер, всплеск ошибок, обвал активности. Не выдумывай «кризис пульса» и не зови «восстанавливать конвейер» — это ложная тревога, которая уводит мир от денег.
...
async def _run_schetchik(period_h: int) -> str:
    ...
    idle_n = sum(1 for a in agents if a.status == "idle")
    lines.append(f"\nАгенты ({len(agents)}, из них в покое/idle: {idle_n} — это норма):")
    for a in agents:
        if a.last_pulse:
            mins = int((datetime.utcnow() - a.last_pulse).total_seconds() / 60)
            pulse_info = f"пульс {mins} мин назад"
        else:
            pulse_info = "пульса ещё не было"
        note = " · в покое, ждёт задач (норма)" if a.status == "idle" else ""
        lines.append(f"  [{a.status}] {a.name} — {pulse_info}{note}")
    lines.append("\nЗадачи Техника (очередь полна по устройству мира — код строят Шеф+Моз, не паниковать):")
    for status, cnt in tasks_by_status:
        lines.append(f"  {status}: {cnt}")
    if last_foreman:
        lines.append(f"\nБригадир Стройки: в очереди {last_foreman.stuck_count} задач (структурно, не авария)")
...
async def _trigger_council_from_analytics(analysis: str) -> None:
    ...
    topic = await _llm(
        ...
        "Приоритет — РОСТ и ДОХОД: продукты, клиенты, монетизация, рынок, реклама. "
        "ЗАПРЕЩЕНО предлагать темы про «восстановление пульса», «разблокировку конвейера», «стабилизацию ядра» "
        "или внутренний ремонт — idle-агенты и очередь Техника это норма, а не проблема. Думай про деньги, не про мнимый кризис.",
