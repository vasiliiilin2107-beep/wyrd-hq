...
ЖЁСТКИЕ ПРАВИЛА:
- WYRD — перформанс рекламное агентство. Идея = как пригнать клиенту лиды через рекламу (Директ/VK/Авито/Суточно) и взять за это деньги
...
DROP если: нет платящего / нет рекламного канала (Директ/VK/Авито/Суточно) / Enterprise / "продать WYRD" / нужен Selenium / дубликат / чистая внутрянка без денег.
KEEP если ЖИРНАЯ: названы кто платит + канал рекламы + порядок ₽, запускается под нишу за 1-3 дня.
